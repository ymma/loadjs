...
class LoadYS {
    ...
    async init() {
        const indexData = await this._fetch(this.release_domain + '/' + this.product + '/' + this.module + '/index.json');
    ...
    async getList(name, { page = 1 } = {}) {
        ...
        const data = await this._fetch(this.release_domain + d.paths[page - 1]);
